...
Snapshot considerato per questo audit: repository `main` al 2026-09-05; le capability riportano PR/evidenze puntuali e non vengono promosse sulla base di descrizioni o vecchie chat. Le funzioni dei prodotti satellite restano al livello dimostrabile dal Core o dal codice presente in questo repository.
...
| Dashboard utente personalizzata | Performance operative, risultati commerciali, obiettivi giornata/30 giorni, budget individuale, obiettivo extra, premi motivazionali, Inbox, task, chiamate e card configurabili dal tenant | Codice | PR #388, #390 e #399. Le superfici sono tenant-scoped; risultati commerciali e premi rispettano i permessi delle aree sottostanti. Collaudo admin + collaboratore reale, premi reali e recupero storico verificato restano il gate prima di `Tenant reale`. Vedere `docs/PERSONALIZED_USER_DASHBOARD.md` e `docs/OPERATOR_GOAL_REWARDS.md`. |
...
| Premi obiettivi operatori | Premio per KPI in punti o EUR, livello extra, progresso personale, conferma admin, ledger per ciclo e distinzione approvato/liquidato | Codice | PR #399. Regole e ledger sono backend-only, tenant-scoped, auditabili e idempotenti; nessun pagamento o payroll parte automaticamente. Per `Tenant reale` servono prova con due utenti, punti, premio economico liquidato, deduplica ciclo, livello extra, permessi collaboratore e isolamento fra due tenant. Vedere `docs/OPERATOR_GOAL_REWARDS.md`. |
...
| Centralini telefonici provider-agnostic | Scelta PBX per tenant, registry provider, adapter, verifica connessione, click-to-call e guide di configurazione | Codice | PR #425. Il Core non dipende piu da 3CX per la scelta PBX/click-to-call; catalogo iniziale: 3CX, Wildix, NethVoice, VOIspeed, Yeastar, Teams Phone, Webex Calling, Asterisk/FreePBX e Avaya IP Office. Verifica automatica implementata per 3CX/Wildix/NethVoice/VOIspeed/Yeastar/Asterisk; click-to-call per 3CX/VOIspeed/Yeastar/Asterisk. Teams/Webex restano guidati e Avaya richiede bridge. Solo 3CX conserva evidenze reali precedenti: gli altri richiedono E2E su impianto cliente prima di `Tenant reale`. Vedere `docs/TELEPHONY_PROVIDER_ARCHITECTURE.md`. |

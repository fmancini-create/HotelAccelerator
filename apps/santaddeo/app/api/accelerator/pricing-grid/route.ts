...
async function handleGET(request: NextRequest) {
  try {
    ...
    // FIX 21/07/2026: `total` e' la capacita' NETTA (total_rooms - fuori
    // servizio), non quella lorda. Tutti i consumatori (questa UI + motore
    // pricing) calcolano `sold = total - available`: se `total` fosse lordo, le
    // camere in ferie/manutenzione (che abbassano rooms_available ma NON sono
    // vendute) verrebbero contate come vendute -> occupancy gonfiata (incident
    // Moriano feb-2027: 9/10 mentre le 9 erano fuori servizio). Con `total`
    // netto: sold = (total_rooms - oos) - available = camere realmente occupate,
    // e il denominatore = capacita' realmente vendibile. La formula a valle
    // resta invariata (nessun tocco al motore triplicato).

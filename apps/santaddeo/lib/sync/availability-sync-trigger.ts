...
export async function triggerAvailabilitySyncForDates(
  ...
  if (!bypassLock) {
    try {
      const acquired = await redis.set(lockKey, triggeredBy, {
        nx: true,
        ex: LOCK_TTL_SECONDS,
      })
      if (acquired !== "OK") {
  ...
  try {
    ...
    try {
      await redis.del(`santaddeo:cache:availability:${hotelId}`)
      await redis.del(`santaddeo:cache:occupancy:${hotelId}`)
    } catch {

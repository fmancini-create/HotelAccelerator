...
export class ScidooMapper {
  ...
  mapAvailability(rawAvailability: any) {
    ...
    // FIX 23/05/2026 #2 (incident Moriano - 88 cam "vendute" Nov + 10 Dic in
    // pagina Obiettivi senza prenotazioni reali). Quando Scidoo manda
    // available=0 + occupied=0 + oos=0 e noi ricostruiamo total_rooms dalla
    // capacita' canonica del room_type, il calcolo a valle
    //   sold = total_rooms - rooms_available - rooms_out_of_service
    // diventa `sold = total - 0 - 0 = total` (tutto venduto), pur non
    // esistendo nessuna prenotazione: in realta' Scidoo segnala un giorno
    // di blocco totale (chiusura programmata, manutenzione, ecc.).
    // Patch: se sumCount=0 ma c'e' fallback capacity, marca tutta la
    // capacita' ricostruita come OOS, cosi' a valle sold=0 (corretto) e
    // l'occupancy% non sara' falsata.
    const reconstructedFromFallback =
      !rawAvailability.total_rooms && sumCount === 0 && fallbackCapacity > 0
    const roomsOutOfService = reconstructedFromFallback
      ? totalRooms
      : oosFromScidoo

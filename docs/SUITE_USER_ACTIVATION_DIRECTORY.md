...
Il browser invia esclusivamente `product` ed `externalUserId`. Il Core:
...
3. usa email e nome ricevuti server-to-server, ignorando eventuali dati client;
4. crea/riusa la `suite_identity` nello stesso customer account;
5. collega l'identita locale del satellite;
6. crea o riusa l'utente HotelAccelerator;
7. collega l'identita HA.
...
Il rollout non modifica lo schema dati e non intercetta flussi login esistenti. Per rollback operativo si puo rimuovere/nascondere la sezione directory dal Core; gli endpoint satellite sono sola lettura e possono restare presenti senza modificare utenti o accessi esistenti.

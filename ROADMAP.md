...
- [ ] Assistente vocale AI per telefonate e messaggi vocali con handoff umano e contesto preservato. Bridge 3CX → basi tenant-aware, mappa persistente degli otto percorsi 4BID, fallback espliciti, template CRM v3 e sync firmato da documenti interni sono in stato `Codice`. Per il raro PBX condiviso 4BID/Villa I Barronci il Core usa mapping opt-in + hint autenticato e cattura live delle chiamate solo-bot; restano applicazione migrazione, collaudo reale dei due numeri dallo stesso chiamante, verifica transcript→domanda, fonti dei satelliti e osservabilità prima di promuovere lo stato.

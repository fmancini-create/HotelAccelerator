import { type NextRequest, NextResponse } from "next/server"

import { authenticateRegistryClient } from "@/lib/customer-codes/registry-auth"
import { assertExistingHotelAcceleratorPropertyUsable } from "@/lib/suite-identity/preflight"
import { provisionHotelAcceleratorFromSatellite, SuiteIdentityError } from "@/lib/suite-identity/registry"
import { parseSuiteSsoProduct } from "@/lib/suite-sso/config"

export const dynamic = "force-dynamic"
export const runtime = "nodejs"

function response(body: unknown, status = 200) {
  return NextResponse.json(body, { status, headers: { "Cache-Control": "no-store" } })
}

export async function POST(request: NextRequest) {
  const body = (await request.json().catch(() => null)) as {
    product?: unknown
    externalTenantId?: unknown
    externalUserId?: unknown
    email?: unknown
    name?: unknown
    isTenantAdmin?: unknown
    tenantName?: unknown
  } | null

  const product = parseSuiteSsoProduct(body?.product)
  const externalTenantId = typeof body?.externalTenantId === "string" ? body.externalTenantId.trim() : ""
  const externalUserId = typeof body?.externalUserId === "string" ? body.externalUserId.trim() : ""
  const email = typeof body?.email === "string" ? body.email.trim().toLowerCase() : ""
  const name = typeof body?.name === "string" ? body.name.trim() : null
  const tenantName = typeof body?.tenantName === "string" ? body.tenantName.trim() : ""
  const isTenantAdmin = body?.isTenantAdmin === true

  if (!product || !externalTenantId || !externalUserId || !email || !tenantName) {
    return response({ error: "Richiesta non valida" }, 400)
  }

  const auth = await authenticateRegistryClient(
    product,
    request.headers.get("x-4bid-registry-key"),
    request.headers.get("authorization"),
  )
  if (!auth.ok) {
    return response(
      { error: auth.configured ? "Non autorizzato" : "Autenticazione non configurata" },
      auth.configured ? 401 : 503,
    )
  }

  try {
    await assertExistingHotelAcceleratorPropertyUsable({ product, externalTenantId })
    const result = await provisionHotelAcceleratorFromSatellite({
      product,
      externalTenantId,
      externalUserId,
      email,
      name,
      isTenantAdmin,
      tenantName,
    })
    return response(result)
  } catch (error) {
    if (error instanceof SuiteIdentityError) {
      return response({ error: error.message, code: error.code }, error.status)
    }
    console.error("[suite-identity] satellite launch failed", {
      product,
      external_tenant_id: externalTenantId,
      error: error instanceof Error ? error.message : "unknown",
    })
    return response({ error: "Provisioning suite non disponibile" }, 500)
  }
}

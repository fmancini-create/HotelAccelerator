...
export async function GET(request: NextRequest) {
  ...
  const exportUrl = new URL(`${SUITE_SSO_CONFIG.santaddeo.baseUrl.replace(/\/$/, "")}/api/integrations/support/v1/export`)
  ...
  try {
    ...
    if (!response.ok) return NextResponse.json({ error: "Santaddeo export unavailable" }, { status: 502 })

...
/**
 * Error boundary diagnostico per la rimappatura PMS (/settings/pms).
 *
 * Bug 02/05/2026: durante la rimappatura di Podere Casanova al termine del
 * wizard appariva il fallback globale generico "Qualcosa e' andato storto".
 * Con il boundary globale non si vedeva ne' messaggio ne' stack trace,
 * impossibile diagnosticare. Questo boundary di segmento mostra:
 *   - error.message
 *   - error.name
 *   - error.digest
 *   - stack trace completa (pretichiusa di default, espandibile)
 * cosi' l'utente puo' copiarli e incollarli e individuiamo subito il punto.
 *
 * Resta safe: non altera nessun flusso, viene attivato SOLO quando React
 * lancia un'eccezione non catturata nel rendering della pagina /settings/pms.
 */
...
export default function SettingsPmsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (error) {
      console.error("[v0] /settings/pms error:", error.message || error)
      console.error("[v0] /settings/pms stack:", error.stack)
      console.error("[v0] /settings/pms digest:", error.digest)
    }
  }, [error])

  const diagnosticText =
    `Pagina: /settings/pms (rimappatura PMS)\n` +
      ...
      <div className="max-w-2xl w-full bg-white rounded-xl border shadow-sm p-6">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-gray-900 mb-1">Errore durante la rimappatura PMS</h1>
            <p className="text-sm text-muted-foreground">
              Si e&apos; verificato un errore imprevisto in <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">/settings/pms</code>.

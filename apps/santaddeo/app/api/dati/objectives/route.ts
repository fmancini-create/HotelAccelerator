  ...
  try {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      if (rmsHasRows) {
        ...
        //
        // FIX 21/07/2026 (capacita' reale RMS, non teorica): prima si usava
        // `theoreticalCapacity` (= dailyCapacityFromRoomTypes * lastDay), cioe'
        // la somma STATICA di TUTTE le room_types con total_rooms>0 IGNORANDO
        // is_active e la data. Per Barronci = 27/giorno (24 attive + 3 tipologie
        // disattivate: 2 Appartamenti + Villa in esclusiva) -> 837/mese IDENTICO
        // 2025 e 2026, nonostante gli Appartamenti siano stati disattivati dal
        // 12/04/2026. La RMS (fonte certa, come /accelerator/price) sa gia' la
        // verita': quelle tipologie hanno total_rooms=0 dopo la disattivazione e
        // 0 righe in tutto il 2025 -> capacita' reale ~24/giorno. Usiamo quindi
        // la capacita' REALE sommata dalla RMS (`rmsCapacityMonth`), sempre al
        // netto delle notti OOS, coerente col numeratore venduto (anch'esso RMS).
        // theoreticalCapacity resta solo come sanity-cap per non superare mai la
        // capacita' fisica massima. Impatta capacita'/occupancy/RevPAR di TUTTI
        // gli hotel (alza le occupancy prima sotto-stimate dalle tipologie
        // fantasma), scelta confermata dall'utente.
        totalRoomsCapacity = Math.max(
          0,
          Math.min(rmsCapacityMonth, theoreticalCapacity) - totalMonthOos,
        )
      ...
      if (prevRmsHasSufficientData) {
        ...
        // 27/06/2026 FIX (occupancy PY >100%): quando il numeratore "camere
        // vendute" e' derivato dai booking SENZA filtro per room_type (include
        // le tipologie oggi disattivate ma vendute l'anno scorso) mentre il
        // denominatore RMS copre solo le tipologie tracciate -> vendute >
        // capacita' -> occupancy oltre il 100%. In quel caso si alza la base alla
        // capacita' fisica (tetto certo delle notti vendibili).
        //
        // FIX 21/07/2026 (capacita' reale RMS anche sul PY): dopo il fix del
        // numeratore PY (prevYearTotalSold usa prevRmsSold, RMS-based, per i mesi
        // trascorsi e per la modalita' Totale), numeratore e denominatore vengono
        // dalle STESSE righe RMS -> il tetto fisico non serve piu' e anzi gonfiava
        // la capacita' a quella teorica (Barronci 27/giorno = 24 attive + 3
        // tipologie disattivate), rendendo Cam.Disponibili 2025 == 2026. Percio':
        // se il venduto PY e' RMS-based usiamo la capacita' RMS REALE
        // (rmsGrossCapacity, riflette gli Appartamenti disattivati e lo storico
        // reale); solo quando il venduto resta booking-based (mesi PY non ancora
        // trascorsi in modalita' Ad Oggi) manteniamo la guardia anti >100%.
        const prevSoldIsRmsBased =
          prevRmsHasSufficientData &&
          prevRmsSold > 0 &&
          (!filterAdOggi || prevMonthFullyElapsedAtCutoff)
        const grossCapacity = prevSoldIsRmsBased
          ? rmsGrossCapacity
          : Math.max(rmsGrossCapacity, physicalCapacityPrevMonth)

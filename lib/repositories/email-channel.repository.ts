...
/**
 * Data Access Layer for Email Channels
...
export class EmailChannelRepository {
  ...
  /**
   * Find channel by email address and property
   */
  async findByEmail(propertyId: string, emailAddress: string): Promise<EmailChannel | null> {
    const { data, error } = await this.supabase
      .from("email_channels")
      .select("*")
      .eq("property_id", propertyId)
  ...
  /**
   * List assignments for a channel
   */
  ...
  /**
   * Set assignments for a channel (replaces all)
   */

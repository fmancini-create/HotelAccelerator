...
export class SuperAdminRepository {
  ...
  async getAllStructures(): Promise<StructureWithStats[]> {
    // Fetch properties with admin count
    const { data, error } = await this.supabase.from("properties").select("*").order("created_at", { ascending: false })

    if (error) throw error

    // Map properties con campi minimi per la lista
    const structures = (data || []).map((property: any) => ({
      ...property,
      status: property.subscription_status || "active", // Map subscription_status to status
      users_count: 0, // Non serve nella lista, verrà caricato solo nel dettaglio
      admin_count: 0,
  ...
  async getStructureById(id: string): Promise<Structure | null> {
    ...
    return {
      ...data,
      status: data.subscription_status || "active", // Map subscription_status to status
      users_count: data.admin_users?.[0]?.count || 0, // Add users count
    } as Structure
  }

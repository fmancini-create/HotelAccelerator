-- Preserve the existing Inbox filter semantics when full-text search is active.
-- Production migration version: 20260906002021.

drop function if exists public.search_inbox_conversation_ids(uuid, text, text, text, text, text, text, boolean, uuid[], uuid[], text, integer, integer);

create function public.search_inbox_conversation_ids(
  p_property_id uuid,
  p_search text,
  ...
  matched as (
    ...
    where c.property_id = p_property_id
      ...
      and (
        ...
        or (p_filter = 'action_needed' and coalesce(c.metadata->'intelligence_summary'->'next_action'->>'action', '') not in ('', 'await_response', 'none'))
      )
      and (
...
revoke all on function public.search_inbox_conversation_ids(uuid, text, text, text, text, text, text, text, boolean, uuid[], uuid[], text, integer, integer) from public, anon;
grant execute on function public.search_inbox_conversation_ids(uuid, text, text, text, text, text, text, text, boolean, uuid[], uuid[], text, integer, integer) to authenticated, service_role;

comment on function public.search_inbox_conversation_ids(uuid, text, text, text, text, text, text, text, boolean, uuid[], uuid[], text, integer, integer) is 'Tenant-scoped indexed Inbox search with current status/channel/folder/complex-filter semantics.';

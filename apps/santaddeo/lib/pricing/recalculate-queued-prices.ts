...
export async function recalculatePricesForQueuedItem(queueItem: {
  ...
  try {
    ...
    const [
      ...
      fetchAllPaginated<{ date: string; room_type_id: string; rooms_available: number; total_rooms: number; rooms_out_of_service: number }>(
        () =>
          supabase
    ...
    // Build maps
    // FIX 21/07/2026: `total` = capacita' NETTA (total_rooms - fuori servizio),
    // allineato alla UI (app/api/accelerator/pricing-grid). Il motore calcola
    // sold = total - available e l'incremento per fascia di occupazione su
    // totalSold/totalCap: con `total` lordo le camere in ferie/manutenzione
    // venivano contate come vendute -> occupancy gonfiata -> prezzi alzati nei
    // periodi di chiusura (incident Moriano). Con `total` netto sold = camere
    // realmente occupate e la capacita' = quella vendibile. Formula invariata.
    ...
    for (const row of availability) {
      ...
      const oos = Math.max(0, row.rooms_out_of_service ?? 0)
      const netTotal = Math.max(0, (row.total_rooms ?? 0) - oos)

...
/**
 * Error boundary diagnostico per /settings/mappings (HotelMappingsViewer +
 * GdocsRoomTypeMapping + RateMappingEditor).
 *
 * Vedi /settings/pms/error.tsx per la motivazione completa: senza questo
 * boundary di segmento non si vedeva ne' il messaggio ne' lo stack quando
 * un componente client-side crashava durante la rimappatura, e cadevamo
 * sul fallback globale generico "Qualcosa e' andato storto".
 */
...
export default function SettingsMappingsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (error) {
      console.error("[v0] /settings/mappings error:", error.message || error)
      console.error("[v0] /settings/mappings stack:", error.stack)
      console.error("[v0] /settings/mappings digest:", error.digest)
    }
  }, [error])

  const diagnosticText =
    `Pagina: /settings/mappings\n` +
      ...
      <div className="max-w-2xl w-full bg-white rounded-xl border shadow-sm p-6">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-gray-900 mb-1">Errore nella pagina Mapping</h1>
            <p className="text-sm text-muted-foreground">
              Si e&apos; verificato un errore imprevisto in <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">/settings/mappings</code>.

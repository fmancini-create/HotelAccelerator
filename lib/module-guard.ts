...
export async function checkModuleEnabled(
  ...
  try {
    ...
    return checkModuleEnabledForProperty(propertyId, flag)
  } catch (err) {
...
/**
 * Verifica che un modulo sia abilitato dato un propertyId gia' autenticato.
 * Usare quando getAuthenticatedPropertyId() e' gia' stato chiamato.
 */
export async function checkModuleEnabledForProperty(
  propertyId: string,
  flag: ModuleFlag
): Promise<NextResponse | null> {

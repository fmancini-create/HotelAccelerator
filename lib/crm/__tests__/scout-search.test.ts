...
describe("Scout guided taxonomy", () => {
  it("offers the nautical and workplace-safety choices used in the demo", () => {
    expect(SCOUT_INDUSTRY_OPTIONS.some((option) => option.id === "nautical" && option.searchTerm === "settore nautico")).toBe(true)
    expect(SCOUT_ROLE_OPTIONS.some((option) => option.id === "workplace-safety" && option.searchTerm === "responsabile della sicurezza")).toBe(true)
    expect(SCOUT_LOCATION_OPTIONS.some((option) => option.id === "it" && option.searchTerm === "Italy")).toBe(true)
  })
})

describe("interpretScoutSearch", () => {
  it("expands the nautical occupational-safety demo search", () => {
    const { providerInput } = search({ keywords: "settore nautico", titles: ["responsabile della sicurezza"] })
    expect(providerInput.keywords.split(",")).toEqual(expect.arrayContaining(["maritime", "marine", "yachting", "shipbuilding"]))
    expect(providerInput.titles).toEqual(expect.arrayContaining(["safety manager", "HSE manager", "QHSE manager", "RSPP"]))
  ...
  it("keeps cybersecurity separate from workplace safety", () => {
    ...
    expect(providerInput.titles).toEqual(expect.arrayContaining(["cyber security manager", "information security manager", "CISO"]))
  ...
  it("preserves unknown industry and role terms", () => {
    const { providerInput } = search({ keywords: "microbirrifici", titles: ["mastro birraio"], organizationLocations: ["Tuscany, Italy"] })

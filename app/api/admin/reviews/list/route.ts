...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const upstream = await forwardReviewsList({
    ...
    console.error("[reviews] native list failed", { error: error instanceof Error ? error.message : "unknown" })
    return response({ error: "reviews_list_failed" }, 500)
  }
}

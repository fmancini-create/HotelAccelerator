...
export async function projectFederatedSupport(snapshot: FederatedSupportSnapshot) {
  ...
  const lastMessageAt = snapshot.messages
  ...
  const metadata = {
    support_federation: {
      ...
      customer_property_id: resolved.propertyId,
...
export async function createHotelAcceleratorSupportReport(input: {
  ...
  const context = [
    input.currentPath ? `Pagina: ${input.currentPath}` : null,
    input.actorName || input.actorEmail ? `Segnalato da: ${input.actorName || input.actorEmail}` : null,
    input.description,
  ].filter(Boolean).join("\n\n")

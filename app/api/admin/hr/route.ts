...
import { AccessError, accessErrorStatus, adminUserIdPerDatabase, requireTenantAdmin } from "@/lib/auth/admin-access"
...
const schema=z.discriminatedUnion("action",[
 ...
 z.object({action:z.literal("shift"),employee_id:z.string().uuid(),date:z.string().date(),starts:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),ends:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),break_minutes:z.coerce.number().int().min(0).max(720).default(0),location:z.string().max(120).optional()}),
...
export async function GET(req:NextRequest){try{const {identity,db}=await context(req);const from=new URL(req.url).searchParams.get("from")||new Date().toISOString().slice(0,10);
 ...
 // due liste divergevano. Nota: `user_groups` non ha `is_active`, quindi qui non
 // si filtra (filtrare su una colonna inesistente farebbe fallire tutto il GET).
...
export async function POST(req:NextRequest){try{const {identity,db}=await context(req);const parsed=schema.safeParse(await req.json());if(!parsed.success)return NextResponse.json({error:"invalid_payload",details:parsed.error.flatten()},{status:400});const b=parsed.data;const property_id=identity.propertyId;
 if(b.action==="department"){
  // Creare un reparto qui significa creare un GRUPPO DI LAVORO: e' la stessa
  // lista della pagina Utenti. Il gruppo nasce senza permessi (il predefinito
  // nega), quindi un reparto senza account - le cameriere ai piani, la cucina -
  // non apre alcun accesso: e' esattamente il caso di F&B e Spa, che oggi hanno
  // zero membri e nessun effetto sui permessi.
  const q=await db.from("user_groups").insert({property_id,name:b.name,color:"#6b7280"}).select("id,name,color").single();
  // 23505 = UNIQUE (property_id, name): il nome esiste gia'. Non e' un guasto ed
  // e' la ragione per cui la lista unica funziona; rispondere 500 avrebbe fatto
  // credere a un errore del sistema invece di "quel reparto c'e' gia'".
  if(q.error&&(q.error as {code?:string}).code==="23505")return NextResponse.json({error:"department_exists"},{status:409});
  if(q.error)throw q.error;return NextResponse.json(q.data,{status:201})}

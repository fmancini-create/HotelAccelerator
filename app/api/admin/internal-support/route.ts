...
export async function POST(request: NextRequest) {
  ...
  if (parsed.data.action === "report") {
    ...
    try {
      ...
      console.error("[internal-support] report creation failed", { error: error instanceof Error ? error.message : "unknown" })
  ...
  try {
    ...
    const context = baseIds.length > 0
      ? await retrieveContext(baseIds, `${parsed.data.question}\nPagina corrente: ${parsed.data.current_path || "non indicata"}`, { matchCount: 8, minSimilarity: 0.2 })
      : []

    const knowledge = context.map((chunk, index) => `[${index + 1}] ${chunk.content}`).join("\n\n").slice(0, 24000)
    ...
    return NextResponse.json({ answer: text.trim() || "Non ho trovato informazioni sufficienti nella guida interna per rispondere con precisione.", grounded: context.length > 0 })
  } catch (error) {
    console.error("[internal-support] guide failed", { error: error instanceof Error ? error.message : "unknown" })

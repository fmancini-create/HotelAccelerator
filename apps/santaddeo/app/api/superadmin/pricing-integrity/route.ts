/**
 * API per il banner di allerta integrita' pricing nella dashboard superadmin.
 *
 *   GET  -> lista alert APERTI (resolved_at IS NULL) da pricing_integrity_alerts.
 *   POST -> risolve manualmente un alert ({ id }): imposta resolved_at + resolved_by.
 *
 * Popolata dal cron /api/cron/pricing-integrity. Solo super_admin.
 */
...
export async function GET() {
  ...
  try {
    ...
    console.error("[pricing-integrity API] GET error:", error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("[pricing-integrity API] POST error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}

...
| Gestione conversazioni | Assegnazione, stati, priorità, tag, note, SLA, template, allegati, ricerca, traduzione | Codice | La Inbox unificata applica filtro canale e sottocanale/account server-side con scope tenant e assegnazioni utente; barra KPI personale riusa l'opt-in `operator_kpi_settings` e il calcolo affidabile delle performance. Typecheck Core, lint dei file coinvolti, dashboard manifest e build Next.js verificati sulla PR #315; completare audit delle restanti funzioni conversazione e collaudo tenant reale prima di promuovere lo stato |
...
| KPI operatori | Risposte, conversazioni e attesa mediana per operatore, con opt-in tenant per singolo utente | Codice | Misurazione Inbox solo dalla data di attivazione; la Inbox mostra ora i KPI self-only usando la stessa fonte della dashboard quando l'utente è abilitato. Conversione, qualità, chiamate e storico precedente restano non disponibili finché non esistono eventi affidabili |
